...
public class BoomerangEntity extends Projectile {
    ...
    public void tick() {
        ...
        if(tickCount - flyingSoundPlayed > 40) {
            level().playSound(null, this.position().x, this.position().y, this.position().z, SoundEvents.PLAYER_ATTACK_SWEEP, SoundSource.NEUTRAL);
    ...
    protected void handleReturningState() {
        ...
        if (ownerEntity == null) {
            ...
            if(timeAlive > Config.BOOMERANG_LIFESPAN.get() || this.getBoundingBox().intersects(ownerEntity.getBoundingBox().inflate(0.2d))) {
                this.moving = false;
    ...
    protected void handleTimeoutState() {
        if(getOwner() == null) {
            if(tickTimeout++ > Config.BOOMERANG_OWNER_TIMEOUT.get()) {
                this.nextState = BoomerangState.RETURNED;
            }
        } else {
            tickTimeout = 0;
    ...
    public void onHitBlock(BlockHitResult hitResult) {
        ...
        level().playSound(null, soundLocation.x, soundLocation.y, soundLocation.z, SoundEvents.STONE_HIT, SoundSource.NEUTRAL);
    ...
    public void onHitEntity(EntityHitResult hitResult) {
        ...
        Holder<Enchantment> knockback = level().registryAccess()
                .lookupOrThrow(Registries.ENCHANTMENT)
                .getOrThrow(Enchantments.KNOCKBACK);
        Holder<Enchantment> piercing = level().registryAccess()
                .lookupOrThrow(Registries.ENCHANTMENT)
        ...
        int unbreakingLevel = boomerangItemStack.getEnchantmentLevel(unbreaking);
        double chance = 1d / (unbreakingLevel + 1);

        if (level() instanceof ServerLevel serverLevel)
        {
            this.boomerangItemStack.hurtAndBreak(serverLevel.getRandom().nextDouble() <= chance ? 1 : 0, serverLevel, (LivingEntity) getOwner(), (brokenItem) ->
            {
                this.discard();
                serverLevel.playSound(null, getX(), getY(), getZ(),
                        SoundEvents.ITEM_BREAK, SoundSource.PLAYERS, 1.1f, 0.8f + random.nextFloat() * 0.4f);
            });
        }
        ...
        int knockbackLevels = boomerangItemStack.getEnchantmentLevel(knockback);
        if(knockbackLevels > 0) {
            Vec3 knockbackVector = this.getDeltaMovement().scale(knockbackLevels);
            hitEntity.push(knockbackVector.x, knockbackVector.y, knockbackVector.z);
        }

        int piercingLevels = boomerangItemStack.getEnchantmentLevel(piercing);
        if(++entitiesPierced > piercingLevels && !level().isClientSide()) {
            this.bounceOffEntity(hitEntity);
        }

        int fire = boomerangItemStack.getEnchantmentLevel(fireAspect);
        if(fire > 0) {
            hitEntity.igniteForSeconds(4 * fire);
        }

    }

    public void bounceOffBlock(Direction.Axis surfaceHitAxis) {
        switch (surfaceHitAxis.getName()) {
            case "y" -> this.setDeltaMovement(this.getDeltaMovement().multiply(1.0d, -1.0d, 1.0d).scale(0.8d));
            case "x" -> this.setDeltaMovement(this.getDeltaMovement().multiply(-1.0d, 1.0d, 1.0d).scale(0.8d));
            case "z" -> this.setDeltaMovement(this.getDeltaMovement().multiply(1.0d, 1.0d, -1.0d).scale(0.8d));
        }
    }
    ...
    protected void addAdditionalSaveData(CompoundTag tag) {
        Boomerang.LOGGER.debug("Saving Boomerang (owner): " + this.ownerUUID);
    ...
    protected void readAdditionalSaveData(CompoundTag tag) {
        ...
        Boomerang.LOGGER.debug("Reading Boomerang (owner): " + this.ownerUUID + "\n" + "Owner entity: " + getOwner());
